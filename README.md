...
The status endpoint determines the displayed color. Red takes priority over yellow,
then green. Widget startup is handled separately by `traffic-light-launcher.js`,
which registers no status-event hooks and does not change the transport protocol.
...
The startup-only companion is installed alongside it as
`~/.config/opencode/plugins/traffic-light-launcher.js`. It invokes the installed
CLI once per application process. Managed service commands pass
`--exit-on-disconnect` to the frontend; direct frontend launches stay open by default.
...
Status coordination is owned by `traffic-light.js`, independently of the startup
companion and desktop services. Restart all backends after updating that component
so every process uses the same implementation.
...
node --check traffic-light-launcher.js
...
CI runs shared checks on Ubuntu and macOS, with native compilation/tests when the
native source and test files are present. Installer checks compile a minimal source
fixture so packaging can be tested independently from frontend changes. Runtime
test files are discovered automatically as components are added. The Linux widget
uses GTK/Cairo doubles in its tests. Physical desktop interaction,
...
rm -f "$CONFIG_DIR/plugins/traffic-light-launcher.js"
```
...
Remove the flat `traffic-light-launcher.js` companion for legacy installations too.
