...
def stub(command, args):
    ...
    if command == "uname":
        ...
        if args[0] == "-create":
            ...
            if args[1] == "ProgramArguments.0":
                value["ProgramArguments"].insert(0, item)
            else:
                value[args[1]] = item
...
class InstallTests(unittest.TestCase):
    def setUp(self):
        ...
        self.repo.mkdir()
        for name in ("setup.sh", "traffic-light", "traffic-light.js", "package.json",
                     "opencode-traffic-light.py", "opencode-traffic-light.service",
                     "opencode-traffic-light.swift"):
            if (ROOT / name).exists():
    ...
    def test_macos_fresh_setup_and_repeat(self):
        ...
        self.assertEqual(plist["ProgramArguments"], [str(executable)])
        ...
        self.run_script("setup.sh")
        self.assertEqual(len(list((self.config / "plugins").glob("*.js"))), 1)
