...
function harness() {
  const world = { now: 0, owner: null, processes: [], timers: [], actions: [] };
  ...
  world.load = async (options = {}) => {
    const p = {
      ...
      requests: [], writes: [], logs: [], output: [], timeouts: [],
...
test("initialization binds one loopback server and unrefs protocol handles", async () => {
  const w = harness();
  const p = await w.load();
  assert.deepEqual(w.actions, [`serve:${p.id}`]);
...
test("duplicate initialization and ordinary activity keep one heartbeat per process", async () => {
  const w = harness();
  const p = await w.load();
  ...
  assert.equal(p.writes.length, 1);
  await w.load();
  assert.equal(w.timers.filter((t) => t.interval).length, 2);
});

test("logging and marker failures do not interrupt status updates", async () => {
  for (const options of [
    { writeError: true }, { logThrows: true }, { logRejects: true },
  ]) {
    const w = harness();
...
test("hung logging does not block initialization or status hooks", { timeout: 2000 }, async () => {
  const w = harness();
  const p = await w.load({ logHangs: true });
...
test("background network and bind errors stay quiet and retry", async () => {
  const w = harness();
  const owner = await w.load();
  const peer = await w.load();
