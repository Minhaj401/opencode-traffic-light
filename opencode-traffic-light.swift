...
final class StatusPoller {
    ...
    func poll() {
        ...
        guard now() - lastValidResponse < Self.disconnectGrace else {
            stop()
            onDisconnect()
            return
        }
...
final class WidgetController: NSObject, NSApplicationDelegate {
    ...
    init(preferencesURL: URL = WidgetPreferences.url()) {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        let poller = StatusPoller(onDisconnect: { [weak self] in self?.close() }) {
